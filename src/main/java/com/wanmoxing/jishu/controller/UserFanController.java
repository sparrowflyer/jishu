...
public class UserFanController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getFanCount", method = RequestMethod.POST)
	public ResultDTO getFanCount (HttpSession session, @RequestBody GetFansDTO getFansDTO) {
		ResultDTO result = new ResultDTO();
		try {
			List<User> fansUsers = userFanService.findFansUsers(getFansDTO.getOwnerId());
			result.setData(fansUsers != null ? fansUsers.size() : 0);
			return result;
		} catch (Exception e) {
			e.printStackTrace(); 
			result.setStatus(ResultDTOStatus.ERROR.getStatus());
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getFanedCount", method = RequestMethod.POST)
	public ResultDTO getFanedCount (HttpSession session, @RequestBody JSONObject jsonParams) {
		ResultDTO result = new ResultDTO();
		try {
			String fanId = jsonParams.getString("fanId");
			if (StringUtil.isEmpty(fanId)) {
			...
			List<User> fanedUsers = userFanService.findFanedUsers(Integer.valueOf(fanId));
			result.setData(fanedUsers != null ? fanedUsers.size() : 0);
			return result;
		} catch (Exception e) {
			e.printStackTrace(); 
			result.setStatus(ResultDTOStatus.ERROR.getStatus());
